...
MCQController.horizontal.getOptionLayout = function (layout, question) {
    ...
    console.log("Hey",question.data.options);
    return '<div class="outer-option-container ' + layout + '">\
            <div class="option-container ' + layout + '">\
            <div class="option-block-container">\
            <% _.forEach(question.data.options,function(val,key){ %>\
                <div class="option-block org-ekstep-questionunit-mcq-option-element<% if(val.isCorrect) { %> mcq-correct-answer<% } %>" id="option_<%= key %>" onclick="MCQController.horizontal.onSelectOption(this, <%= key %>);MCQController.horizontal.onOptionSelected(event,<%= key %>)">\
