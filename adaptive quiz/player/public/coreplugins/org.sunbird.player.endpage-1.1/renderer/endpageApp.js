...
endPage.controller("endPageController", function($scope, $rootScope, $state,$element, $stateParams, $http) {
    ...
    $scope.handleEndpage = function() {
       ...
       // alert("calling")
        console.log($rootScope.teacherCode)
        if($rootScope.flag){
                var data = {
                request:{
                    "teacherCode":$rootScope.teacherCode,
                    "courseName":"MATH",
                    "courseCode":"CMATH2",
                    "status":"Completed",
                ...
                $http.post("http://localhost:9081/registry/course/certification", data).then(
